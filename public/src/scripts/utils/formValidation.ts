import {errorMessages, validateEmail, validatePassword} from './validations.js';
  ...
  validate(event: Event) {
    ...
    if (event.target) {
      if (this.checkInputValidity(<HTMLInputElement>event.target)) {
        return this.activateError(<HTMLInputElement>event.target);
      }
      return this.resetError(<HTMLInputElement>event.target);
    }
  }
