...
import { logoutTC } from '../../states/AuthState';
...
export class ProfileButtons extends Freact<IObj> {
  ...
  componentDidMount(oldProps: IObj) {
    // FIXME: СРЕДНИЙ. edit и changePassword не используются в самой компоненте, но приходится их сюда прокидывать,
    //  чтобы они обновились. Надо придумать, как одновлять компоненту извне
    observe.call(this, ProfileState, 'edit');
    observe.call(this, ProfileState, 'changePassword');

    const callback = (ind: number) => function click() {
